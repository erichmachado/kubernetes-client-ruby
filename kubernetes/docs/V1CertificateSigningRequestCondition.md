...
| ---- | ---- | ----------- | ----- |
| **last_transition_time** | **Time** | lastTransitionTime is the time the condition last transitioned from one status to another. If unset, when a new condition type is added or an existing condition&#39;s status is changed, the server defaults this to the current time. | [optional] |
| **last_update_time** | **Time** | lastUpdateTime is the time of the last update to this condition | [optional] |
...
instance = Kubernetes::V1CertificateSigningRequestCondition.new(
